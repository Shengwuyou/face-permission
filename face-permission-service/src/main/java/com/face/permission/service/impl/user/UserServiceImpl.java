...
//        request.
    ...
    public TokenDTO login(UserLoginDTO request) {
        ...
        if (request.getType() != UserEnums.LoginTypeEnum.LOGIN_NAME.getCode()){
            ...
            AssertUtil.isTrue( userDO != null , "登陆账号异常");
            uId = userDO.getuId();
        ...
        userInfo.setUid(userDO.getuId());
        String rls = accountDO.getRoles();
        //将String数组转成Integer数组
        userInfo.setRoles((Integer[]) ConvertUtils.convert(rls.split(","), Integer.class));
        userInfo.setNickName(userDO.getNickName());
        userInfo.setFromWay(request.getFromWay());
        userInfo.setPlatform(request.getPlatform());
        String token = null;
        try {
            token = JwtUtils.createJWT(1, JwtUtils.objectToMap(userInfo), DEFAULT_TOKEN_EXPIRE_TIME);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private PAccountDO getIfAbsentAccount(String uId, String loginName, String password){
        ...
        AssertUtil.isTrue( accountDO != null , "登陆异常");
